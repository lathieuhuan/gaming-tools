...
export class CharacterCalc extends CalcCharacter implements IEffectPerformer {
  ...
  initTotalAttr() {
    ...
    // ===== Artifacts =====
    this.finalizeArtifactAttrs().entries.forEach(([stat, value]) => {
      totalAttrCtrl.applyBonus({
        toStat: stat,
        value,
        label: "Artifact stat",
  ...
  override getTotalAttr(key: AttributeStat | "base_atk", stableOnly = false) {
    return this.totalAttrCtrl.getTotal(key, stableOnly);
  }

  finalizeArtifactAttrs() {
    const { totalAttrCtrl } = this;

    return this.artifact.finalizeAttributes({
