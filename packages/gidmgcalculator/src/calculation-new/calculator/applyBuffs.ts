...
export function applyBuffs(main: CharacterCalc, teammates: TeammateCalc[], setup: ICalcSetup) {
  ...
  function applyBonus(
    ...
    support: Omit<Partial<BonusPerformTools>, "basedOnStable">,
    ...
    for (const config of Array_.toArray(effects)) {
      ...
      if (
        ...
        const basedOnStable = !Array.isArray(targets) && targets.module === "ATTR";
  ...
  function applyArtifactBonuses(isFinalStage: boolean) {
    ...
    for (const { bonusLv, pieceCount, data } of artifact.sets) {
