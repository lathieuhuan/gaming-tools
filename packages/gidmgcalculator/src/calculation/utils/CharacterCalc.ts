...
export class CharacterCalc {
  ...
  static getTalentDefaultInfo(
    ...
    const resultKey =
      expectedAttPatt === "ES" || expectedAttPatt === "EB" ? expectedAttPatt : "NAs";
  ...
  static isGrantedEffect = (grantedAt: CharacterMilestone | undefined, character: Character) => {
    if (grantedAt) {
      const [prefix, level] = grantedAt;
      return (
        (prefix === "A" ? GeneralCalc.getAscension(character.level) : character.cons) >= +level
      );
    }
    return true;
  };

  static reachedPartyMs = (partyMs: PartyMilestone | undefined, teamData: CalcTeamData) => {
    if (partyMs === "SECRET_RITE") {
      return teamData.witchRiteLv >= 2;
    }

    return false;
  };

  static isGrantedMod = (condition: IsGrantedModCondition, teamData: CalcTeamData) => {
    if (condition.partyMs && !this.reachedPartyMs(condition.partyMs, teamData)) {
      return false;
    }

    return this.isGrantedEffect(condition.grantedAt, teamData.activeMember);
  };
}

type IsGrantedModCondition = {
