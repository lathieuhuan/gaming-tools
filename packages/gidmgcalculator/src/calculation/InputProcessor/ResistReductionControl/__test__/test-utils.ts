...
export class ResistanceReductionControlTester extends ResistReductionControl<MutableTeamDataTester> {
  ...
  __expectReduct(key: ResistReductionKey, value: number) {
    expect(this["reductCounter"].get(key)).toBe(value);
  }

  __expectReducts(keys: ResistReductionKey[], value: number) {
    const remainKeys = new Set<ResistReductionKey>([...ATTACK_ELEMENTS, "def"]);
