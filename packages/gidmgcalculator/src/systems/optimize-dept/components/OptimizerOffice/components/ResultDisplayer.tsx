...
export type ResultDisplayerProps = {
  processedResult: ProcessedResult;
  selectingResult: boolean;
  setups: ProcessedSetup[];
  maxSelected: number;
  onLoadSetups: (setups: ProcessedSetup[]) => void;
...
export function ResultDisplayer({
  ...
  const handleArtifactSelect: ResultItemDisplayerProps["onArtifactSelect"] = (artifact) => {
    setSelectedArtifact(artifact);
    if (bodyRef.current) bodyRef.current.scrollLeft = 999;
  };

  const handleSelectSetup = (setup: ProcessedSetup) => (checked: boolean) => {
          ...
          {processedResult.map((processedItem, index) => {
            return (
              <ResultItemDisplayer
                ...
                keepCheckbox={
                  selectingResult ? (
                    <Checkbox defaultChecked onChange={handleSelectSetup(processedItem.manageInfo)}>
                      Keep
                    </Checkbox>
                  ) : null
                }
            ...
            <ButtonGroup
              ...
              buttons={[
                ...
                },
                {
                  ...
                  onClick: () => onLoadSetups(selectedSetups),
                },
              ]}
            />
          </div>
