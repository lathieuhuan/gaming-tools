...
export type ConfirmButtonProps = Pick<ButtonProps, "className" | "variant" | "disabled" | "icon" | "children"> & {
  asking: boolean;
  ctaText: string;
...
export function ConfirmButton({
  ...
  const triggerRef = useClickOutside<HTMLDivElement>(() => onAskingChange(false));

  const handleClick = () => {
    const parent = triggerRef.current?.parentElement;

    // Check the trigger's position relative to its parent to determine the popover position
    ...
    <div ref={triggerRef} className="relative">
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <CloseButton className="shrink-0" boneOnly onClick={() => onAskingChange(false)} />
        </div>
