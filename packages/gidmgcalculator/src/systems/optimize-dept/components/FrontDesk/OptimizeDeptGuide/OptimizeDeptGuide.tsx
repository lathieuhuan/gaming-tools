...
import type { Notify, OnStepChange, StepStatus } from "./types";
...
export type OptimizeDeptGuideProps<T extends string> = {
  active: boolean;
  stepConfigs: StepConfig<T>[];
  canShowMenu?: boolean;
  onStepChange?: OnStepChange<T>;
  onClose: () => void;
};
...
export function OptimizeDeptGuide<T extends string>(props: OptimizeDeptGuideProps<T>) {
  ...
  const navigate = (toStep: number) => {
    ...
    props.onStepChange?.(stepConfigs[toStep].key, stepConfigs[step].key, {
