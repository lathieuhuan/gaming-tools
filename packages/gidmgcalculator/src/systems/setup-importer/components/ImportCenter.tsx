...
import { pickBasicCharProps, pickBasicTargetProps } from "./_utils";
...
export function SetupImportCenter({ params, onFinish, ...manageInfo }: SetupImportCenterProps) {
  ...
  const { character, target, setupManagers } = useShallowCalcStore((state) => {
    const { activeId = "", setupsById } = state;

    return {
      character: setupsById[activeId]?.char,
      target: state.target,
  ...
  useEffect(() => {
    ...
    if (character.name !== params.char.name) {
    ...
    const currentChar = pickBasicCharProps(character);
    const importedChar = pickBasicCharProps(params.char);
    const currentTarget = pickBasicTargetProps(target);
    const importedTarget = pickBasicTargetProps(params.target);
    const sameChar = isEqual(currentChar, importedChar);
