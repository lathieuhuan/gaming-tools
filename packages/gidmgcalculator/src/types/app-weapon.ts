import type { EntityBonus, EntityBuff } from "./modifier-specs";
import type { AttributeStat, CalcItemBasedOn, CalcItemType, WeaponType } from "./common";

export type AppWeapon = {
  /** This is id */
  code: number;
  beta?: boolean;
  type: WeaponType;
  name: string;
  rarity: number;
  icon: string;
  mainStatScale: string;
  subStat?: {
    type: AttributeStat;
    scale: string;
  };
  passiveName?: string;
  descriptions?: string[];
  bonuses?: EntityBonus[];
  buffs?: WeaponBuff[];
  calcItems?: WeaponCalcItem[];
};

type WeaponCalcItem = {
  name: string;
  /** Default 'attack' */
  type?: CalcItemType;
  value: number;
  /** Default 1/3 [value] */
  incre?: number;
  /** Default 'atk' */
  basedOn?: CalcItemBasedOn;
};

// ========== BONUS ==========

export type WeaponBuff = EntityBuff & {
  /**
   * If number, it's the index of weapon's descriptions (AppWeapon.descriptions).
   * Default 0.
   */
  description?: number | string | (number | string)[];
};
