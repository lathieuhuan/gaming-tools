...
export function decodeSetup(code: string): DecodeSuccessResult | DecodeFailResult {
  ...
  const split = (str: string | undefined, splitLv: number) => {
    return str?.split(DIVIDER[splitLv]) || [];
  };
  ...
  try {
    ...
    const teammates = Array_.truthify([
      decodeTeammate(tmStr1),
      decodeTeammate(tmStr2),
      decodeTeammate(tmStr3),
    ]);
    ...
    const decodeElement = (index: number = -1) => {
      return ELEMENT_TYPES[index] || null;
    };
    ...
    const elmtEvent: ElementalEvent = {
      ...
      infusion: decodeElement(+infusion),
      infuseReaction: infuseReaction as AttackReaction,
      absorption: decodeElement(+absorption),
      absorbReaction: absorbReaction as AttackReaction,
