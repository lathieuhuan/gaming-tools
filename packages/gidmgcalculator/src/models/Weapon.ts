...
export class Weapon implements Clonable<Weapon> {
  ...
  static create(ID: number, type: WeaponType, data: AppWeapon, options: WeaponCreateOptions = {}) {
    const { refi = Weapon.#DEFAULT_REFI, level = Weapon.#DEFAULT_LEVEL } = options;

    const { validLevel, bareLv, ascension, mainStatValue, subStatValue } = deriveStateFromLevel(
      level,
      data,
    );
...
function deriveStateFromLevel(level: Level, data: AppWeapon) {
  const { level: validLevel, bareLv, ascension } = parseLevel(level, data.rarity);
  const mainStatValue = BASE_ATTACK_TYPE[data.mainStatScale]?.[LEVELS.indexOf(validLevel)] || 0;
  ...
  if (subStat) {
    ...
    subStatValue = SUBSTAT_SCALE[subStat.scale]?.[index] || 0;
  }
