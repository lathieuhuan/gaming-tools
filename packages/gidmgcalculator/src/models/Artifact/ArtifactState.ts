...
import type { ArtifactSubStat, ArtifactType, AttributeStat, ArtifactStateData } from "@/types";

const allMainStatTypesCache = {} as Record<ArtifactType, AttributeStat[]>;
...
export class ArtifactState implements ArtifactStateData {
  type: ArtifactType;
  rarity: number;
  level: number;
  ...
  get mainStatValue(): number {
    return ArtifactState.mainStatValueOf(this);
  }
  ...
  update(changes: Partial<ArtifactStateData>) {
    Object_.patch<ArtifactState>(this, {
      type: changes.type,
      level: changes.level,
      rarity: changes.rarity,
      mainStatType: changes.mainStatType,
      subStats: changes.subStats ? Object_.clone(changes.subStats) : undefined,
    });
  ...
  protected correctData() {
    const mainStat = mainStatTypeByAtfType(this.type);

    if (!(this.mainStatType in mainStat.map)) {
      this.mainStatType = mainStat.default;
    }
  }

  // ===== STATIC =====

  static mainStatValueOf(artifact: ArtifactStateData) {
  ...
  static allMainStatTypesOf(type: ArtifactType): AttributeStat[] {
    const cached = allMainStatTypesCache[type];
    if (cached) return cached;

    const mainStatTypes = Object_.keys(ARTIFACT_MAIN_STATS[type] || {});
    allMainStatTypesCache[type] = mainStatTypes;

    return mainStatTypes;
  }
}
