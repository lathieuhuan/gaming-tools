...
> extends AbstractEffectValueCalc<TPerformer> {
  //
  protected basedOnStable = false;
  protected refi = 0;

  constructor(
    protected performer: TPerformer,
    protected team: ITeam,
    { inputs = [], refi = 0, basedOnStable = false }: Partial<BonusPerformTools>
  ) {
    super(performer, team, inputs);

    this.refi = refi;
    this.basedOnStable = basedOnStable;
  }
  ...
  protected applyExtra(bonus: BareBonus, config?: number | EntityBonusEffect) {
    if (typeof config === "number") {
      ...
      if (extra) {
        ...
        // if extra is unstable, this whole bonus is unstable
        if (extra.isUnstable) bonus.isUnstable = true;
      }
    }
  }
  ...
  makeBonus(config: EntityBonusEffect): BareBonus {
    ...
    if (config.basedOn) {
      ...
      if (basedOn.field !== "base_atk") bonus.isUnstable = true;
    }
