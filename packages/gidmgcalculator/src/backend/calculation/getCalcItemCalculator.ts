...
export default function getCalcItemCalculator(
  ...
  function genAttPattCalculator(patternKey: AttackPattern) {
    ...
    const level = CharacterCalc.getFinalTalentLv({ ...calcInfo, talentType: resultKey });

    function configFlatFactor(factor: CalcItemFlatFactor) {
    ...
    function configMultFactor(factor: CalcItemMultFactor) {
      const {
        root,
    ...
    function calculate(item: CalcItem, elmtModCtrls: InternalElmtModCtrls): CalculationFinalResultItem {
      ...
      const calculator = genCalculator(type, attPatt, attElmt, item.id);
      ...
      tracker?.recordCalcItem(resultKey, item.name, record);
