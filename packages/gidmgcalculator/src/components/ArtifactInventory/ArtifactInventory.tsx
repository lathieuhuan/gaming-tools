...
import { UserArtifact } from "@/models/userdb";
...
export type ArtifactInventoryProps = Pick<ArtifactFilterProps<UserArtifact>, "forcedType"> &
...
const ArtifactInventoryCore = ({
  ...
  const artifacts = useStoreSnapshot((state) => {
    ...
    if (forcedType) {
      const results: UserArtifact[] = [];

      for (const artifact of userArtifacts) {
        if (artifact.type === forcedType) {
          results.push(new UserArtifact(artifact, setData.get(artifact.code)));
        }
      }
    ...
    return userArtifacts.map((artifact) => new UserArtifact(artifact, setData.get(artifact.code)));
  });
  ...
  const onChangeItem: InventoryRackProps<UserArtifact>["onChangeItem"] = (item) => {
    if (!item) {
