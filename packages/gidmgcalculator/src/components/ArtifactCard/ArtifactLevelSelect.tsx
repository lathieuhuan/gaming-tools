import { cn, VersatileSelect } from "rond";

type ArtifactLevelSelectProps = {
  className?: string;
  mutable?: boolean;
  rarity: number;
  level: number;
...
export function ArtifactLevelSelect({
  ...
  if (!mutable) {
    return (
      <p
        className={cn(
          "px-2 pt-2 pb-1.5 text-lg text-rarity-${rarity} leading-none font-bold rounded bg-dark-3",
          className,
        )}
      >
        +{level}
  ...
  const options = Array.from({ length: maxLevel / 4 + 1 }, (_, i) => {
    const lv = i * 4;
    return {
      label: `+${lv}`,
      value: lv,
