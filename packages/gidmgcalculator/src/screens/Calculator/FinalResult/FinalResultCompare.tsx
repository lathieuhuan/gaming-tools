...
function useLayoutProps(comparedIds: number[]): UseLayoutPropsReturn {
  ...
  const standardWeapon = setupsById[standardId].char.weapon.code;
  const showWeaponCalc = comparedIds.some(
    (id) => setupsById[id].char.weapon.code !== standardWeapon
  );
  ...
  for (const talentType of TALENT_TYPES) {
    const levels = setupIds.map((id) => setupsById[id].char.getFinalTalentLv(talentType));
  ...
  return {
    character: setupsById[standardId].char,
    showWeaponCalc,
    headerConfigs,
    setupIds,
    standardId,
