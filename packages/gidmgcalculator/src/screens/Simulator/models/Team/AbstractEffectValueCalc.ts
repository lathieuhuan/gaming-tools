...
export type EffectToParseText = Pick<AbilityBuff, "description" | "effects">;
...
export abstract class AbstractEffectValueCalc {
  constructor(protected performer: Member, protected team: Team, protected inputs: number[] = []) {}
  ...
  protected getInputIndex(spec: InputStackSpec) {
    return spec.index ?? 0;
  }
  ...
  getStacks(spec?: EffectStackSpec): Stacks | null {
    ...
    const { inputs, performer, team } = this;
    const { memberList, elmtCount } = team;
    ...
    switch (spec.type) {
      ...
      case "MIX": {
        stacks = team.getMixedCount(performer.data.vision);
        break;
      }
      default:
  ...
  parseAbilityText({ description, effects }: EffectToParseText) {
